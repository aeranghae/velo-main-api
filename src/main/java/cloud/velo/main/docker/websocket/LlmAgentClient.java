...
public class LlmAgentClient extends TextWebSocketHandler {
    ...
    public LlmAgentClient(DockerAgentService dockerAgentService, ObjectMapper objectMapper,
                          String userId, String uuid, String baseImage) {
    ...
    public void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        if (executionTurnCount > MAX_TURN_LIMIT) {
            ...
            // 공정 실패 알림 전송 후 소켓 종료
            AiModelMessage.Observation errorObs = new AiModelMessage.Observation("ERROR", 1, "", "MAX_TURN_LIMIT_EXCEEDED: 에이전트 루프가 너무 오래 지속되어 안전을 위해 강제 종료되었습니다.");
        ...
        if ("WRITE_FILE".equals(action.getTool())) {
            try {
                ...
                observation = new AiModelMessage.Observation("SUCCESS", 0, "파일 작성 성공", "");
            } catch (Exception e) {
                log.error("[소켓-{}] 파일 작성 중 오류 발생", uuid, e);
                observation = new AiModelMessage.Observation("ERROR", 1, "", e.getMessage());
            }
        }
            ...
            try {
                ...
                if (isDeleted) {
                    observation = new AiModelMessage.Observation("SUCCESS", 0, "파일 삭제 성공", "");
                } else {
                    observation = new AiModelMessage.Observation("ERROR", 1, "", "파일 삭제 실패 (파일이 존재하지 않거나 잠겨있음)");
                }
            } catch (Exception e) {
            ...
            if (this.registeredContainerId == null) {
                observation = new AiModelMessage.Observation("ERROR", 1, "", "샌드박스 컨테이너가 가동 중이 아닙니다.");
            } else {
            ...
            observation = new AiModelMessage.Observation("ERROR", 1, "", "지원하지 않는 도구입니다.");
        }
